...
const testimonialImages: StaticImageData[] = [
  TestimonialImg01,
  TestimonialImg02,
  TestimonialImg03,
  TestimonialImg04,
...
const clientImages: StaticImageData[] = [
  ClientImg01,
  ClientImg02,
  ClientImg03,
  ClientImg04,
...
const mergedTestimonials = content.testimonials.items.map((testimonial, index) => ({
  ...testimonial,
  img: testimonialImages[index % testimonialImages.length],
  clientImg: clientImages[index % clientImages.length],
}));

export default function Testimonials() {
  const masonryContainer = useMasonry();
  const [category, setCategory] = useState<number>(content.testimonials.categories[0]?.id ?? 1);

  const testimonialsToRender = useMemo(() => mergedTestimonials, []);

  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6" id="risorse">
        ...
        <div>
          ...
          >
            {testimonialsToRender.map((testimonial, index) => (
              <div key={`${testimonial.name}-${index}`} className="group">
...
function categoryIcon(position: number, isActive: boolean) {
  const baseClass = isActive ? "text-indigo-500" : "text-gray-600";
  const icons = [
    (
      <svg
        className={`fill-current ${baseClass}`}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm font-medium text-gray-200">
            ...
            <a
              className="text-indigo-200/65 transition-colors hover:text-indigo-500 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-indigo-400"
              href="#0"
            >
              {testimonial.company}
            </a>
